...
public class EditorActivity extends AppCompatActivity implements LoaderManager.LoaderCallbacks<Cursor> {
    ...
    private int mMode = 0; // 0 = insert, 1 = update
...
//        Uri petUri = (Uri) extras.get("uri");
        if (petUri != null) {
            //The key argument here must match that used in the other activity
            setTitle(getString(R.string.edit_pet_lbl));
            mMode = 1;

            Bundle bundle = new Bundle();
            bundle.putString("uri",petUri.toString());
            getLoaderManager().initLoader(PET_LOADER, bundle, this);

            // Invalidate the options menu, so the "Delete" menu option can be hidden.
            // (It doesn't make sense to delete a pet that hasn't been created yet.)
        } else {
            setTitle(getString(R.string.add_pet_lbl));
    ...
    private void insertPet() {
        ...
        Uri rowId = getContentResolver().insert(PetEntry.CONTENT_URI, values);
    ...
    private void updatePet(){
        ...
        int rowUpdated = getContentResolver().update(petUri, values, null, null);

        if (rowUpdated != 0) {
            Toast.makeText(this, R.string.editor_update_pet_success, Toast.LENGTH_LONG).show();
        } else {
            Toast.makeText(this, R.string.editor_update_pet_fail, Toast.LENGTH_LONG).show();
        }
    }
    /**
...
//    private int getIndex(Spinner spinner, Integer myGender)
//    {
//        int index = 0;
//
//        for (int i=0;i<spinner.getCount();i++){
//            if (spinner.getItemAtPosition(i) == myGender){
//                index = i;
//                break;
//            }
//        }
